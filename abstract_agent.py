...
class Agent():
    def __init__(self, gym_env, net, obs_processing_func, memory_buffer_size, batch_size, learning_rate, gamma,
    ...
    def train(self, number_episodes = 50000, max_steps_episode = 10000, max_steps=1000000, writer_name="default_writer_name"):
        ...
        for ep in tqdm(range(number_episodes), unit=' episodes'):
            ...
            for s in range(max_steps):
                ...
                A = np.argmax(<Q[(position, velocity)]>)
                if np.random.uniform() < self.compute_epsilon(total_steps):
